...
procedure DrawPNG(Png: TPngImage; Canvas: TCanvas; const ARect: TRect;
  const Options: TPngOptions);
procedure ConvertToPNG(Source: TGraphic; out Dest: TPngImage);
procedure CreatePNG(Color, Mask: TBitmap; out Dest: TPngImage;
  InverseMask: Boolean = False);
procedure CreatePNGMasked(Bitmap: TBitmap; Mask: TColor; out Dest: TPngImage);
procedure CopyImageFromImageList(Dest: TPngImage; ImageList: TCustomImageList;
  Index: Integer);
procedure SlicePNG(JoinedPNG: TPngImage; Columns, Rows: Integer;
...
procedure MakeImageBlended(Image: TPngImage; Amount: Byte = 127);
  ...
  procedure ForceAlphachannel(BitTransparency: Boolean;
...
begin
  ...
  if not Forced and (Image.Header.ColorType in [COLOR_RGBALPHA,
    COLOR_GRAYSCALEALPHA]) then
  begin
    for Y := 0 to Image.Height - 1 do
    begin
...
var
  ...
  function CompareColors(const Color1: TRGBTriple;
    const Color2: TColor): Boolean;
  begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        HasAlpha := False;
        for I := 0 to (SourceColor.Height * SourceColor.Width) - 1 do
        begin
  ...
  begin
    ...
    Result := (Info.dwPlatformId = VER_PLATFORM_WIN32_NT) and
      ((Info.dwMajorVersion > 5) or ((Info.dwMajorVersion = 5) and
    ...
    begin
      ...
      begin
        ...
        if TBitmap(Source).TransparentMode = tmFixed then
          ...
          TransparentColor := TBitmap(Source).Canvas.Pixels
            [0, Source.Height - 1];
...
begin
  ...
  begin
    Line := Dest.AlphaScanline[Y];
    for X := 0 to Dest.Width - 1 do
      Line[X] := Integer(TColor(GetPixel(Bitmap.Canvas.Handle, X, Y)) <>
        Mask) * $FF;
  end;
end;
    ...
    try
      ...
      begin
        ...
        begin
          AlphaLine := Dest.AlphaScanline[Y];
          for X := 0 to Dest.Width - 1 do
            AlphaLine^[X] := Integer(GetPixel(MaskBitmap.Canvas.Handle, X, Y)
              = COLORREF(clBlack)) * $FF;
        end;
      end;
    finally
      ...
      try
        ...
        begin
          ...
          for X := 0 to Bitmap.Width - 1 do
            BitmapLine[X] := ColorToTriple(JoinedPNG.Pixels[X + OffsetX,
              Y + OffsetY]);
        end;
