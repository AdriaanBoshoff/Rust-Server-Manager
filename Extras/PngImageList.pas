...
{$IFEND}
  ...
  uses Windows, Classes, SysUtils, Controls, Graphics, ImgList, PngImage,
  PngFunctions;
...
type
  ...
  protected
    ...
    procedure InternalInsertPng(Index: Integer; const Png: TPngImage;
      Background: TColor = clNone);
    procedure InternalAddPng(const Png: TPngImage; Background: TColor = clNone);
    function PngToIcon(const Png: TPngImage;
      Background: TColor = clNone): HICON;
  ...
  public
    ...
    procedure InsertPng(Index: Integer; Image: TPngImage;
      Background: TColor = clNone);
    procedure InsertMasked(Index: Integer; Image: TBitmap;
      MaskColor: TColor); virtual;
    ...
    procedure ReplaceMasked(Index: Integer; NewImage: TBitmap;
      MaskColor: TColor); virtual;
  published
    property EnabledImages: Boolean read FEnabledImages write SetEnabledImages
      default True;
    property Height read GetHeight write SetHeight default 16;
    property PngImages: TPngImageCollectionItems read FPngImages
      write SetPngImages;
    property PngOptions: TPngOptions read FPngOptions write SetPngOptions
      default [pngBlendOnDisabled];
  ...
  public
    ...
    function Add(DontCreatePNG: Boolean = False): TPngImageCollectionItem;
      reintroduce;
    procedure Assign(Source: TPersistent); override;
    function Insert(Index: Integer; DontCreatePNG: Boolean = False)
      : TPngImageCollectionItem; reintroduce;
    property Items[index: Integer]: TPngImageCollectionItem read GetItem
  ...
  public
    ...
    constructor Create(Collection: TCollection; DontCreatePNG: Boolean = False);
...
function PatchPtr(OldPtr, NewPtr: Pointer; const Name: string;
  Patch: TMethodPatch): Boolean;
var
  Access: Cardinal;
begin
  Result := False;
  ...
  begin
    ...
    if VirtualProtect(OldPtr, 16, PAGE_EXECUTE_READWRITE, @Access) then
    begin
      PByte(OldPtr)^ := $E9; // Near jump
      PCardinal(NativeInt(OldPtr) + 1)^ := NativeInt(NewPtr) -
        NativeInt(OldPtr) - 5;
...
begin
  ...
  begin
    ...
    Pointers[1] := Combo(@TCustomImageList.AddIcon, @TPngImageList.AddIcon,
      'AddIcon');
    Pointers[2] := Combo(@TCustomImageList.AddImage, @TPngImageList.AddImage,
      'AddImage');
    Pointers[3] := Combo(@TCustomImageList.AddImages, @TPngImageList.AddImages,
      'AddImages');
    Pointers[4] := Combo(@TCustomImageList.AddMasked, @TPngImageList.AddMasked,
      'AddMasked');
    Pointers[5] := Combo(@TCustomImageList.Clear, @TPngImageList.Clear,
      'Clear');
    Pointers[6] := Combo(@TCustomImageList.Delete, @TPngImageList.Delete,
      'Delete');
    Pointers[7] := Combo(@TCustomImageList.Insert, @TPngImageList.Insert,
      'Insert');
    Pointers[8] := Combo(@TCustomImageList.InsertIcon,
      @TPngImageList.InsertIcon, 'InsertIcon');
    Pointers[9] := Combo(@TCustomImageList.InsertMasked,
      @TPngImageList.InsertMasked, 'InsertMasked');
    Pointers[10] := Combo(@TCustomImageList.Move, @TPngImageList.Move, 'Move');
    Pointers[11] := Combo(@TCustomImageList.Replace, @TPngImageList.Replace,
      'Replace');
    Pointers[12] := Combo(@TCustomImageList.ReplaceIcon,
      @TPngImageList.ReplaceIcon, 'ReplaceIcon');
    Pointers[13] := Combo(@TCustomImageList.ReplaceMasked,
      @TPngImageList.ReplaceMasked, 'ReplaceMasked');
    ...
    begin
      ...
      begin
        ...
        if PatchPtr(Pointers[I].OldPtr, Pointers[I].NewPtr, Pointers[I].Name,
          Patch) then
...
begin
  Result := False;
  if VirtualProtect(OldPointer, 16, PAGE_EXECUTE_READWRITE, @Access) then
  begin
...
function TPngImageList.AddPng(Image: TPngImage;
  Background: TColor = clNone): Integer;
var
...
function TPngImageList.AddImage(Value: TCustomImageList;
...
procedure TPngImageList.DoDraw(Index: Integer; Canvas: TCanvas; X, Y: Integer;
...
procedure TPngImageList.InsertPng(Index: Integer; Image: TPngImage;
  Background: TColor = clNone);
var
  Item: TPngImageCollectionItem;
begin
...
procedure TPngImageList.InsertMasked(Index: Integer; Image: TBitmap;
  MaskColor: TColor);
var
  Patch: TMethodPatch;
  Png: TPngImage;
...
procedure TPngImageList.InternalAddPng(const Png: TPngImage;
  Background: TColor = clNone);
var
  Icon: HICON;
begin
...
function TPngImageList.PngToIcon(const Png: TPngImage;
  Background: TColor): HICON;
const
...
procedure TPngImageList.ReplaceMasked(Index: Integer; NewImage: TBitmap;
  MaskColor: TColor);
var
  Item: TPngImageCollectionItem;
  Patch: TMethodPatch;
...
function TPngImageCollectionItems.Add(DontCreatePNG: Boolean = False)
...
function TPngImageCollectionItems.GetItem(Index: Integer)
  : TPngImageCollectionItem;
begin
  if (Index >= 0) and (Index < Count) then
...
function TPngImageCollectionItems.Insert(Index: Integer;
  DontCreatePNG: Boolean = False): TPngImageCollectionItem;
begin
  Result := Add(DontCreatePNG);
  Result.Index := Index;
end;

procedure TPngImageCollectionItems.SetItem(Index: Integer;
  const Value: TPngImageCollectionItem);
begin
  if (Index >= 0) and (Index < Count) then
