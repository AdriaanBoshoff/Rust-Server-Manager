...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, Mask, ComCtrls, Generics.Defaults,
...
begin
  TCustomAction(Sender).Enabled := (StyleName <> '') and
    not SameText(StyleName, TStyleManager.ActiveStyle.Name, loUserLocale);
end;
...
begin
  ...
  begin
    ...
    If UpDownSat.Position <> 0 then
      ...
      Result.Add(TBitmap32SaturationFilter.Create
        ((255 - ((UpDownSat.Position) * 255) div MaxSat)));
  ...
  begin
    ...
    try
      RttiInstanceType := (ctx.GetType(CbBlend.Items.Objects[CbBlend.ItemIndex])
        as TRttiInstanceType);
      LFilter := RttiInstanceType.GetMethod('Create')
        .Invoke(RttiInstanceType.MetaclassType, [ColorBoxblend.Selected]);
  ...
  begin
    ...
    try
      RttiInstanceType :=
        (ctx.GetType(CbBlendTextures.Items.Objects[CbBlendTextures.ItemIndex])
        as TRttiInstanceType);
      // ShowMessage(IntToStr(ImageTexture.Picture.Bitmap.Width));
      LFilter := RttiInstanceType.GetMethod('CreateBitMap')
        .Invoke(RttiInstanceType.MetaclassType, [ImageTexture.Picture.Bitmap]);
...
procedure TFrmHueSat.btn1Click(Sender: TObject);
begin
...
begin
  ...
  try
    ...
    StatusBar1.SimpleText :=
      (Format('ellapsed %d ms', [sw.ElapsedMilliseconds]));
    TStyleManager.ReloadStyle(StyleName);
    LoadStyle;
  except
    on E: Exception do
      MessageDlg(Format('Error saving vcl style - Message : %s : Trace %s',
        [E.Message, E.StackTrace]), mtWarning, [mbOK], 0);
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if (TrackBarHue.Position <> 0) or (TrackBarSaturation.Position <> 0) or
        (TrackBarLightness.Position <> 0) then
        NewName := Format('%s H%d.S%d.L%d', [NewName, TrackBarHue.Position,
          TrackBarSaturation.Position, TrackBarLightness.Position]);
    end
    else if RadioButtonRGB.Checked then
      NewName := Format('%s R%d.G%d.B%d', [NewName, TrackBarRed.Position,
        TrackBarGreen.Position, TrackBarBlue.Position])
    else if RadioButtonBlend.Checked then
    begin
      ...
      else if StartsText('clWeb', ColorName) then
    ...
    try
      ...
      try
        ...
        try
          ...
          DrawSampleWindow(VclUtils.StyleExt, LBitmap.Canvas,
            ImageVCLStyle.ClientRect, NewName, Icon.Handle);
          ...
          try
            LPng.SaveToFile(ChangeFileExt('theme', '.png'));
          finally
            LPng.Free;
          end;
        finally
...
begin
  ...
  begin
    ...
    try
      ...
      try
        DrawSampleWindow(LStyle, LBitmap.Canvas, ImageVCLStyle.ClientRect,
          StyleName);
...
begin
  ...
  ComboBoxVclStyles.ItemIndex := ComboBoxVclStyles.Items.IndexOf
...
begin
  ...
  if DirectoryExists('.\Textures') then
  begin
    for s in TDirectory.GetFiles(ExtractFilePath(ParamStr(0)) + 'Textures',
      '*.jpg') do
  ...
  try
    ...
    begin
      ...
      try
        ...
        ListViewStyleColors.Items.Item[Index].ImageIndex :=
          ImageListStyleColors.Count - 1;
      finally
        LBitmap.Free;
      end;
    end;
    ...
    begin
      ...
      try
        ...
        ListViewStyleFontColors.Items.Item[Index].ImageIndex :=
          ImageListStyleFontColors.Count - 1;
      finally
        LBitmap.Free;
      end;
    end;
    ...
    begin
      ...
      try
        ...
        ListViewSystemColors.Items.Item[Index].ImageIndex :=
          ImageListSystemColors.Count - 1;
      finally
        LBitmap.Free;
      end;
    end;
...
begin
  ...
  try
    ...
    if SaveDialog1.Execute then
      TVclStylesUtils.SaveSettings(SaveDialog1.FileName, LElements,
        LVCLStylesFilter, LFilters);
  finally
    LFilters.Free;
  end;
end;
...
begin
  ImageTexture.Picture.LoadFromFile(ExtractFilePath(ParamStr(0)) + 'textures\' +
    CbTextures.Text);
...
begin
  ...
  begin
    ...
    try
      TVclStylesUtils.LoadSettings(OpenDialog1.FileName, LElements, LFilterType,
        LFilters);
        ...
        vsfHSL:
          begin
            ...
            for Filter in LFilters do
              ...
              else if Filter is TBitmap32SaturationFilter then
                TrackBarSaturation.Position := Filter.ColorValue
              else if Filter is TBitmap32LightnessFilter then
        ...
        vsfRGB:
          begin
            ...
            for Filter in LFilters do
              ...
              else if Filter is TBitmap32GreenFilter then
                TrackBarGreen.Position := Filter.ColorValue
              else if Filter is TBitmap32BlueFilter then
      ...
      try
        ...
        on E: Exception do
          MessageDlg(Format('Error applying settings - Message : %s : Trace %s',
            [E.Message, E.StackTrace]), mtWarning, [mbOK], 0);
      end;
...
begin
  ...
  if DirectoryExists(StylesDir) then
    for Style in TDirectory.GetFiles(IncludeTrailingPathDelimiter(StylesDir),
      '*.vsf') do
